			...
			<plugin> <2>
				...
				<dependencies>
					<dependency> <4>
						...
						<version>{project-version}</version> <5>
					</dependency>
				</dependencies>
			</plugin>
		</plugins>
...
<4> Add `spring-restdocs-asciidoctor` as a dependency of the Asciidoctor plugin. This
	...
	block macro.
<5> If you are using along with `spring-boot` then replace version with ${spring-restdocs.version} 
        which handles version upgrading when parent version is upgraded.
