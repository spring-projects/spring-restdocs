...
----
	...
	dependencies {
		testImplementation 'org.springframework.restdocs:spring-restdocs-mockmvc:{project-version}' <2>
	...
	asciidoctor { <6>
		...
		baseDirFollowsSourceDir() <8>
	}
----
<1> Apply the Asciidoctor plugin.

<2> Add a dependency on `spring-restdocs-mockmvc` in the `testImplementation` configuration. If
...
<7> Make the task depend on the test task so that the tests are run before the
    documentation is created.
<8> The base directory will always be the same as the source directory.
    If an intermediate working directory is being used,
    the base directory will automatically point to that.
====
