...
You may want to package the generated documentation in your project's jar file -- for example, to have it {spring-boot-docs}/web/servlet.html#web.servlet.spring-mvc.static-content[served as static content] by Spring Boot.
...
Spring REST Docs uses Spring MVC's {spring-framework-docs}/testing/mockmvc.html[test framework] or Spring WebFlux's {spring-framework-docs}/testing/webtestclient.html[`WebTestClient`] to make requests to the service that you are documenting.
...
MockMvc::
+
include-code::mockmvc/ExampleApplicationTests[]
...
WebTestClient::
+
include-code::webtestclient/ExampleApplicationTests[]
...
See the xref:reference:configuration/index.adoc[configuration section] for more information.
...
You must also call `ManualRestDocumentation.afterTest` after each test.
...
MockMvc::
+
include-code::mockmvc/ExampleApplicationTestNgTests[]

WebTestClient::
+
include-code::webtestclient/ExampleApplicationTestNgTests[]
====
...
MockMvc::
+
include-code::mockmvc/InvokeService[]
...
WebTestClient::
+
include-code::webtestclient/InvokeService[]
<1> Invoke the root (`/`) of the service and indicate that an `application/json` response is required.
<2> Assert that the service produced the expected response.
<3> Document the call to the service, writing the snippets into a directory named `index` (which is located beneath the configured output directory).
...
See xref:reference:documenting-your-api/index.adoc[] for more information about these and other snippets that can be produced by Spring REST Docs.
