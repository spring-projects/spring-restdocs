...
public class ResponseCookiesSnippetTests extends AbstractSnippetTests {
	...
	public void responseWithCookies() throws IOException {
		new ResponseCookiesSnippet(Arrays.asList(
				...
				CookieDocumentation.cookieWithName("Vary").description("six"))).document(
						this.operationBuilder.response().cookie("X-Test", "test")
								.cookie("Content-Type", "application/json")
								.cookie("Etag", "lskjadldj3ii32l2ij23")
								.cookie("Cache-Control", "max-age=0")
								.cookie("Vary", "User-Agent").build());
	...
	public void responseCookiesWithCustomAttributes() throws IOException {
		...
		new ResponseCookiesSnippet(Collections
	...
	public void additionalDescriptors() throws IOException {
		CookieDocumentation
				.responseCookies(
						CookieDocumentation.cookieWithName("X-Test").description("one"),
						CookieDocumentation.cookieWithName("Content-Type")
								.description("two"),
						CookieDocumentation.cookieWithName("Etag").description("three"))
				...
				.document(this.operationBuilder.response().cookie("X-Test", "test")
						.cookie("Content-Type", "application/json")
						.cookie("Etag", "lskjadldj3ii32l2ij23")
						.cookie("Cache-Control", "max-age=0").cookie("Vary", "User-Agent")
						.build());
		assertThat(this.generatedSnippets.responseCookies())
				.is(tableWithHeader("Name", "Description").row("`X-Test`", "one")
						.row("`Content-Type`", "two").row("`Etag`", "three")
