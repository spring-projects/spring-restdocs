...
public class RequestCookiesSnippetTests extends AbstractSnippetTests {
	...
	public void requestWithCookies() throws IOException {
		new RequestCookiesSnippet(Arrays.asList(
				CookieDocumentation.cookieWithName("Session").description("one"),
				...
				CookieDocumentation.cookieWithName("Connection").description("five")))
						.document(this.operationBuilder.request("http://localhost")
								.cookie("Session", "test").cookie("User", "nobody")
								.cookie("Timeout", "3600").cookie("Preference", "inverse")
								.cookie("Connection", "secure").build());
		assertThat(this.generatedSnippets.requestCookies())
				.is(tableWithHeader("Name", "Description").row("`Session`", "one")
						.row("`User`", "two").row("`Timeout`", "three")
						.row("`Preference`", "four").row("`Connection`", "five"));
	}

	@Test
	public void undocumentedRequestCookie() throws IOException {
		new RequestCookiesSnippet(Collections.singletonList(
				CookieDocumentation.cookieWithName("X-Test").description("one")))
						.document(this.operationBuilder.request("http://localhost")
								.cookie("X-Test", "test").cookie("Second", "*/*")
								.build());
		assertThat(this.generatedSnippets.requestCookies())
	...
	public void requestCookiesWithCustomAttributes() throws IOException {
		...
		new RequestCookiesSnippet(Collections
				.singletonList(CookieDocumentation.cookieWithName("X-Test").description(
						"one")),
	...
	public void additionalDescriptors() throws IOException {
		CookieDocumentation.requestCookies(
				...
				CookieDocumentation.cookieWithName("Accept-Encoding")
						.description("three"),
				CookieDocumentation.cookieWithName("Accept-Language").description("four"))
				.and(CookieDocumentation.cookieWithName("Cache-Control")
						.description("five"),
						CookieDocumentation.cookieWithName("Connection")
								.description("six"))
				.document(this.operationBuilder.request("http://localhost")
						.cookie("X-Test", "test").cookie("Accept", "*/*")
						.cookie("Accept-Encoding", "gzip, deflate")
						.cookie("Accept-Language", "en-US,en;q=0.5")
	...
	public void tableCellContentIsEscapedWhenNecessary() throws IOException {
		new RequestCookiesSnippet(Collections.singletonList(
				CookieDocumentation.cookieWithName("Foo|Bar").description("one|two")))
						.document(this.operationBuilder.request("http://localhost")
								.cookie("Foo|Bar", "baz").build());
		assertThat(this.generatedSnippets.requestCookies()).is(
